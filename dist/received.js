...
var nodeData; // obeject we will push to firebase

var fbData; // data we pull from firebase

var fbDataArray; // firebase data values converted to an Array

var database; // reference to our firebase database

var folderName = 'messages'; // name of folder you create in db

var form = document.querySelector('form.send');
var receiveButton = document.querySelector('button.receive');
...
function start() {
  ...
  firebase.initializeApp(configureKey); // store database

  database = firebase.database(); // this references the folder you want your data to appear in

  var ref = database.ref(folderName); // **** folderName must be consistent across all calls to this folder
  // console.log(ref);

  ref.on('value', recievedData, errData); // ---> To find your config object:
...
function recievedData(data) {
  ...
  fbData = data.val();
  fbData ? fbDataArray = Object.values(fbData) : console.log('nothing in this folder yet'); // if (fbData) {
...
function createNode(_nodeFolder, _nodeId, _nodeObject) {
  firebase.database().ref("".concat(_nodeFolder, "/").concat(_nodeId)).set(_nodeObject);
...
function updateNode(_nodeFolder, _nodeID, _updateObject) {
  firebase.database().ref("".concat(_nodeFolder, "/").concat(_nodeId)).update(_updateObject); // this will update existing key:value pair(s) OR add new ones to your object
...
function deleteNode(_nodeFolder, _nodeID) {
  firebase.database().ref("".concat(_nodeFolder, "/").concat(_nodeID)).remove();
}

function sendMessage(e) {
  var messageInput = form.querySelector('textarea[name="message"]');
  var timeStamp = Date.now(); // const validChars = char => char.match(/[a-z1-9 .\n]/i);
  ...
  alert('Thank you! Your message was received. Take care of yourself the best way you can :)');
}

function receiveMessage(e) {
  e.preventDefault();
  var randomNumber = Math.floor(Math.random() * fbDataArray.length);
  var pTag = document.querySelector('.receivedData');
